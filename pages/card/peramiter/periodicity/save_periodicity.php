...
if(isset($formdata[0]['value']))
{
	...
	$duplicate = mysql_query("SELECT value from periodicity where value= '$value' ");
	if(mysql_num_rows($duplicate) > 0)
	{
		echo "SORRY DUPLICATE  VLUE";
	}else{
		$insert = mysql_query("INSERT into  periodicity (value, description) values ('$value', '$description') ");
		if($insert)
		{
