...
if(isset($formdata[0]['value']))
{
	...
	$duplicate = mysql_query("SELECT value from payment_method where value= '$value' ");
	if(mysql_num_rows($duplicate) > 0)
	{
		echo "SORRY DUPLICATE  VLUE";
	}else{
		$insert = mysql_query("INSERT into  payment_method (value, description) values ('$value', '$description') ");
		if($insert)
		{
