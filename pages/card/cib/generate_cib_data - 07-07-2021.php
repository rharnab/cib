...
if(mysql_num_rows($mis_query) > 0)
{
		...
		if(empty($dup_subject_result ))
		{
			...
			if($subject_query)
			{
				...
				if($error_response != 0)
				{
 				...
 				/* end  error function start*/

				/*end find out error for this subject*/
	...
	$cl_query= mysql_query("SELECT branch_name, client_id, card_no, card_status, contract,create_date, exp_date, caredit_limit_bdt , outstanding_bdt, total_outstanding_bdt , mp_due_bdt, reporting_date, deliquency,file_name, overdue_amt_bdt, national_id,dath_of_birth, contract_status from cl_table ct  where year(reporting_date)='$report_year' and month(reporting_date)='$report_month'");
	...
	if(mysql_num_rows($cl_query) > 0)
	{
		...
		while($cl_result = mysql_fetch_array($cl_query))
		{
			...
			if($contracts_query)
			{
				...
				/*find out error for this contract*/
				  $contracts_id = mysql_insert_id();
				 /* error function start*/
				$error_response =errorCheckContact($id=$contracts_id,$reportingDate=NULL);
				$t_error =  count($error_response[$id]);
				$field_array  = array();
				if($error_response != 0)
				{
					for($i=0; $i < $t_error; $i++)
					{
					 ...
					 $error_update= mysql_query("UPDATE contracts_info set error_field = '$error_field' where id= '$id'  ");
					 if($error_update)
					 {
					 	$contract_error += 1;
					 }
...
if(($suject_insert && $contracts_insert))
{
	...
	$generate_timestamp = date('Y-m-d H:i:s');
	 ...
	 if($log_insert)
	 {
	 	/*echo $duplicat_fi_subject_code. " DATA DUPLICATE FOR SUBJECT INFO and 
									 	 CIB DATA SUCCESSFUL CREATE NOW 
									  	 YOU CAN GENERATE TXT FILE";*/


		echo $suject_insert." DATA SUCCESSFULLY GENERATE  ".  $duplicat_fi_subject_code." DATA DIUPLICATE ". $field_error_count." DATA SUBJECT ERROR ".  $contract_error." AND DATA CONTRACT  ERROR ".  "NOW YOU CAND DOWNLOAD CIB REPORT ";
