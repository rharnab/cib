...
*/
	...
	if(mysql_num_rows($cl_query) > 0)
	{
		...
		while($cl_result = mysql_fetch_array($cl_query))
		{
			...
			$fi_code='077';
			//$branch_code='';
			$card_fi_sub_cl = $cl_result['client_id'];
			...
			if($cl_result['national_id'] !='')
			{
				...
				$nid= $cl_result['national_id'];
				
				if(strlen($cl_result['national_id']) ==10 || strlen($cl_result['national_id']) == 13)
				{
			...
			if(!empty($cl_result['national_id']))
			{
				$fi_subject_code_cl = flora_fi_subject_code($nid);
				file_put_contents("cnid_check.txt", $fi_subject_code."\n", FILE_APPEND);
				if($fi_subject_code_cl =='')
				{
				 	//$fi_subject_code_cl = $card_fi_sub_cl;
				 	$fi_subject_code_cl = flora_fi_subject_code($full_nid_number);
				 	 if($fi_subject_code_cl == '')
				 	 {
				 	 	$fi_subject_code_cl = $card_fi_sub_cl;
				 	 }
