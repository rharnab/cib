...
<?php require('error_check_subject.php'); ?>
<?php require('error_check_contact.php'); ?>
...
//$q=mysql_query("SELECT  *, b.record_type as rec1 FROM contracts_info a, subject_info b where a.fi_subject_code=b.card_fi_sub");


/*$q=mysql_query("SELECT  *,si.record_type as rec1 from subject_info si where fi_subject_code in (SELECT distinct fi_subject_code FROM contracts_info where status='0') and year(reporting_date)='$acc_year' and month(reporting_date)='$acc_month' and status='0'  group by si.fi_subject_code");*/


/*$q=mysql_query("SELECT *,si.record_type as rec1 from subject_info si where fi_subject_code in (SELECT distinct fi_subject_code FROM contracts_info where year(reporting_date)='$acc_year' and month(reporting_date)='$acc_month' and status='0') and year(reporting_date)='$acc_year' and month(reporting_date)='$acc_month' and si.status='0' group by si.fi_subject_code");*/

$q=mysql_query(" SELECT *,
			       si.record_type AS rec1
					FROM   subject_info si
					WHERE  card_fi_sub  IN (SELECT DISTINCT card_fi_sub 
					                           FROM   contracts_info
					                           WHERE  Year(reporting_date) = '$acc_year'
					                                  AND Month(reporting_date) = '$acc_month'
					                                  AND status = '0')
					       AND si.status = '0'
					GROUP  BY si.fi_subject_code");
...
//$q=mysql_query("SELECT *, a.record_type as rec2 FROM contracts_info a, subject_info b where a.fi_subject_code=b.card_fi_sub");


/*$q=mysql_query("SELECT  *, c.record_type as rec2 from contracts_info c where c.fi_subject_code in (SELECT distinct fi_subject_code FROM subject_info) and year(reporting_date)='$acc_year' and month(reporting_date)='$acc_month' and c.status=0 ");*/

/*$q=mysql_query("SELECT *, c.record_type as rec2 from contracts_info c where c.fi_subject_code in (SELECT distinct fi_subject_code FROM subject_info WHERE Year(reporting_date) = '$acc_year' AND Month(reporting_date) = '$acc_month' and status = '0')  and Year(c.reporting_date) = '$acc_year' and month(c.reporting_date) = '$acc_month' and c.status='0' ");*/



$q=mysql_query("SELECT *,
		       c.record_type AS rec2
				FROM   contracts_info c
				WHERE  c.card_fi_sub  IN (select distinct si2.card_fi_sub  from subject_info si2 where si2.card_fi_sub  in (select  distinct card_fi_sub  from contracts_info ci
				WHERE  Year(ci.reporting_date) = '$acc_year'
		       AND Month(ci.reporting_date) = '$acc_month'
		       AND ci.status = '0'))
		       AND Year(c.reporting_date) = '$acc_year'
		       AND Month(c.reporting_date) = '$acc_month'
		       AND c.status = '0' ");
...
/*update cib data for contracts and subject*/


/*subjcet_info statsu and code_link update*/
$update_sql = mysql_query("SELECT status,code_link from subject_info si WHERE  card_fi_sub IN (SELECT DISTINCT card_fi_sub
   FROM   contracts_info
   WHERE  Year(reporting_date) = '$acc_year'
          AND Month(reporting_date) = '$acc_month'
          AND status = '0')
       AND si.status = '0'
	GROUP  BY si.card_fi_sub ");
...
//subject update here
mysql_query("UPDATE subject_info si set status = '1', code_link='$sub_code_link' WHERE  card_fi_sub IN (SELECT DISTINCT card_fi_sub FROM   contracts_info  WHERE  Year(reporting_date) = '$acc_year' AND Month(reporting_date) = '$acc_month'
  AND status = '0')  AND si.status = '0' ");

/* end subjcet_info statsu and code_link update*/


/*contracts_info status update here*/
/*mysql_query("UPDATE  contracts_info c set status = '1' WHERE  c.card_fi_sub  IN (select distinct si2.card_fi_sub  from subject_info si2 where si2.card_fi_sub  in (select  distinct card_fi_sub  from contracts_info ci
				WHERE  Year(ci.reporting_date) = '$acc_year'
		       AND Month(ci.reporting_date) = '$acc_month'
		       AND ci.status = '0'))
		       AND Year(c.reporting_date) = '$acc_year'
...
mysql_query("UPDATE contracts_info set status = '1'
WHERE  Year(reporting_date) = '$acc_year'
       AND Month(reporting_date) = '$acc_month'
       AND status = '0'
       AND
       card_fi_sub IN (
 select card_fi_sub from (SELECT DISTINCT card_fi_sub FROM subject_info WHERE card_fi_sub IN(SELECT DISTINCT card_fi_sub FROM contracts_info ci WHERE Year(reporting_date) = '$acc_year' AND Month(reporting_date) = '$acc_month' AND status = '0')) d   
       )");

/* end contracts_info status update here*/

 /*end update cib data for contracts and subject*/  
