...
try {
    ...
    if ($sj1count > $sj2count) {
        file_put_contents($generate_sub_file, $sj1h . PHP_EOL, FILE_APPEND);
        for ($i = 0; $i < count($file_sub_array1); $i++) {
            file_put_contents($generate_sub_file, $file_sub_array1[$i] . PHP_EOL, FILE_APPEND);
        }
        for ($j = 0; $j < count($file_sub_array2); $j++) {
            file_put_contents($generate_sub_file, $file_sub_array2[$j] . PHP_EOL, FILE_APPEND);
        }
        ...
        file_put_contents($generate_sub_file, $sjfooter . PHP_EOL, FILE_APPEND);
    } else {
        file_put_contents($generate_sub_file, $sj2h . PHP_EOL, FILE_APPEND);
        for ($i = 0; $i < count($file_sub_array2); $i++) {
            file_put_contents($generate_sub_file, $file_sub_array2[$i] . PHP_EOL, FILE_APPEND);
        }
        for ($j = 0; $j < count($file_sub_array1); $j++) {
            file_put_contents($generate_sub_file, $file_sub_array1[$j] . PHP_EOL, FILE_APPEND);
        }
    ...
    if ($ct1count > $ct2count) {
        file_put_contents($generate_con_file, $ct1h . PHP_EOL, FILE_APPEND);
        for ($i = 0; $i < count($file_con_array1); $i++) {
            file_put_contents($generate_con_file, $file_con_array1[$i] . PHP_EOL, FILE_APPEND);
        }
        for ($j = 0; $j < count($file_con_array2); $j++) {
            file_put_contents($generate_con_file, $file_con_array2[$j] . PHP_EOL, FILE_APPEND);
        }
        ...
        $sjfooter = str_pad($sj_tot, 7, "0", STR_PAD_LEFT);
        $sjfooter =  $sj_footer_text . $sjfooter;
        file_put_contents($generate_con_file, $sjfooter . PHP_EOL, FILE_APPEND);
    } else {
        file_put_contents($generate_con_file, $ct2h . PHP_EOL, FILE_APPEND);
        for ($i = 0; $i < count($file_con_array2); $i++) {
            file_put_contents($generate_con_file, $file_con_array2[$i] . PHP_EOL, FILE_APPEND);
        }
        for ($j = 0; $j < count($file_con_array1); $j++) {
            file_put_contents($generate_con_file, $file_con_array1[$j] . PHP_EOL, FILE_APPEND);
        }
